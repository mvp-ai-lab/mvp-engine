"""Template for recipe-local skill runtime tests.

Copy this file into ``recipes/<recipe>/skill_tests/<skill-id>/test_runtime.py``
...
def test_recipe_runtime_template_builds_dataset_model_optimizer_and_scheduler(tmp_path: Path) -> None:
    ...
    with single_rank_distributed_env():
        ...
        try:
            ...
            first_tensor = next(value for value in batch.values() if hasattr(value, "shape"))
            assert first_tensor.ndim >= 1

            assert engine.unwrapped_model.__class__.__name__
            assert engine.optimizer.__class__.__name__
            assert engine.scheduler.__class__.__name__
