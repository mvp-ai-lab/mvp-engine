"""Training engine for the interleaved recipe."""
...
from ..configs.schema import InterleavedConfig
from ..dataset import build_interleaved_data_kit
...
class InterleavedEngine(Engine):
    """Recipe-local engine for interleaved image-text training."""

    ConfigClass = InterleavedConfig
    ...
    def __init__(self, config):
        """Initialize interleaved-local distributed state and metric reducers."""
        ...
        self.data_kit = build_interleaved_data_kit()
    ...
    def prepare_dataloader(self, workflow: str = "train"):
        ...
        """
        if workflow != "train":
            logger.warning(f"Interleaved engine does not support workflow '{workflow}'.")
            return

        # Step 1: build the shared processor.
        self.processor = self.data_kit.build_processor(
            self.config.model.pretrained_model_name_or_path,
            image_min_pixels=self.config.model.image_min_pixels,
