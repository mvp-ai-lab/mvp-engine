"""Structure tests for the interleaved recipe.
...
def test_file_structure():
    """Validate the baseline interleaved file layout.
    ...
    EXPECTED_FILES = [
        ...
        "configs/__init__.py",
        "configs/schema.py",
        "configs/debug_wiki_64k.yaml",
        "configs/debug_wiki_cut.yaml",
        "configs/wiki_bee_scaledown_stage2_64k.yaml",
        "configs/wiki_bee_stage2_64k.yaml",
        "configs/wiki_merge_missing0420_stage2_64k.yaml",
        "dataset/__init__.py",
        "dataset/interleaved.py",
        "engine/__init__.py",
        "engine/interleaved_engine.py",
        "model/__init__.py",
        "model/qwen3_vl.py",
...
def test_config_structure():
    """Validate all interleaved YAML configs against the recipe schema.

    Each config is parsed through ``InterleavedConfig`` and its engine name is
    ...
    from recipes.interleaved.configs.schema import InterleavedConfig
    ...
    try:
        for config_file in configs:
            if config_file.endswith(".yaml"):
                ...
                config = InterleavedConfig.model_validate(
