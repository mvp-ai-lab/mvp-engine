"""Pydantic schema for the interleaved recipe."""
...
class InterleavedDataConfig(BaseModel):
    """Dataset and batching options for interleaved image-text training."""
...
class InterleavedGradientCheckpointingConfig(BaseModel):
...
class InterleavedCompileConfig(BaseModel):
    """Model compile options for the interleaved recipe."""
...
class InterleavedModelConfig(BaseModel):
    """Model loading, precision compatibility, and freeze-policy options."""

    model_config = ConfigDict(frozen=False, extra="forbid")

    pretrained_model_name_or_path: str = "Qwen/Qwen3-VL-8B-Thinking"
    attn_implementation: Literal["eager", "sdpa", "flash_attention_2"] = "flash_attention_2"
    image_min_pixels: int | None = Field(1024, ge=1)
    image_max_pixels: int | None = Field(None, ge=1)
    gradient_checkpointing: InterleavedGradientCheckpointingConfig = Field(
        default_factory=InterleavedGradientCheckpointingConfig
    )

    # Full-SFT defaults from the LLaMA-Factory interleaved project.
    freeze_vit: bool = False
    freeze_projector: bool = False
    freeze_llm: bool = False

    compile: InterleavedCompileConfig = Field(default_factory=InterleavedCompileConfig)
...
class InterleavedOptimConfig(BaseOptimConfig):
    """Optimizer options extended with interleaved batch and loss-guard settings."""
...
class InterleavedLoopConfig(BaseLoopConfig):
    """Iteration-loop options for interleaved training."""
...
class InterleavedConfig(BaseEngineConfig):
    """Top-level interleaved recipe config."""

    model_config = ConfigDict(frozen=False, extra="allow")

    data: InterleavedDataConfig = Field(default_factory=InterleavedDataConfig)
    model: InterleavedModelConfig = Field(default_factory=InterleavedModelConfig)
    optim: InterleavedOptimConfig = Field(default_factory=InterleavedOptimConfig)
    loop: InterleavedLoopConfig = Field(default_factory=InterleavedLoopConfig)
