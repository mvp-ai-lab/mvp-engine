"""Miscellaneous helpers for the interleaved recipe."""
...
from mvp_engine.kit.mllm import PackingOptions
...
def infer_total_steps(
    ...
    *,
    ...
    """Infer total optimization steps from one finite interleaved data pass."""
    inference_config = config.model_copy(deep=True)
    data_kit = build_interleaved_data_kit()
    ...
    if global_packed_samples <= 0:
        raise RuntimeError("Interleaved step inference found no packed training samples.")
    ...
    if samples_per_optimization_step <= 0:
        raise RuntimeError(
            "Interleaved step inference cannot infer steps with non-positive samples per optimization step."
        )
