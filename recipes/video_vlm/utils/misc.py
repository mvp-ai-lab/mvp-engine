"""Miscellaneous helpers for the Video VLM recipe."""
...
from ..dataset.collator import VideoVLMCollator
from ..dataset.dataset import build_dataset


def resolve_batching_config(config: VideoVLMConfig, *, data_parallel_world_size: int) -> None:
    """Resolve Video VLM global batch size into micro batch size or accumulation."""
...
def infer_total_steps(
    ...
    *,
    ...
    """Infer total optimization steps from one finite Video VLM data pass."""
    inference_config = config.model_copy(deep=True)

    collate_fn = VideoVLMCollator(
    ...
    if global_packed_samples <= 0:
        raise RuntimeError("Video VLM step inference found no packed training samples.")
    ...
    if samples_per_optimization_step <= 0:
        raise RuntimeError(
            "Video VLM step inference cannot infer steps with non-positive samples per optimization step."
        )
