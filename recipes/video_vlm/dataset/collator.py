"""Batch collation for the Video VLM recipe."""
...
class VideoVLMCollator:
    ...
    def _get_dummy_image(self) -> Image.Image:
        ...
        cached = getattr(self.processor, "_mvp_video_vlm_batch_dummy_image", None)
        if isinstance(cached, Image.Image):
            return cached.copy()

        dummy = Image.new("RGB", self.DUMMY_IMAGE_SIZE, color=0)
        setattr(self.processor, "_mvp_video_vlm_batch_dummy_image", dummy)
        return dummy.copy()
    ...
    def _append_dummy_suffix(self, batch: list[ModelInputs]) -> dict[str, torch.Tensor] | None:
        ...
        if any(
            sample.get("pixel_values") is not None or sample.get("pixel_values_videos") is not None for sample in batch
        ):
            return None
    ...
    def __call__(self, batch: list[ModelInputs]) -> ModelInputs:
        ...
        if any("pack_segment_ids" in sample for sample in batch):
            if not all("pack_segment_ids" in sample for sample in batch):
                raise ValueError("Packed and unpacked samples cannot be mixed in the same video_vlm batch.")
