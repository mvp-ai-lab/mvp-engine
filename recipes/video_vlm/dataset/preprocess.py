...
def convert_images_to_pixel_values(
    ...
    *,
    ...
    """Convert resolved Video VLM images/videos into OneVision tensors.
    ...
    if isinstance(sample, list):
        return [convert_images_to_pixel_values(s, processor=processor, codec_config=codec_config) for s in sample]
    ...
    try:
        ...
        if not videos and sample.get("video") is not None:
        ...
        if images:
            ...
            image_processor = getattr(
                processor, "onevision_image_processor", getattr(processor, "image_processor", None)
            )
            image_inputs = image_processor(
                images=[tvF.to_pil_image(image) for image in resized_images],
                return_tensors="pt",
            )
            grid = int(image_inputs["pixel_values"].shape[-1]) // int(getattr(processor, "onevision_patch_size", 14))
            sample["pixel_values"] = image_inputs["pixel_values"]
            sample["image_grid_thw"] = torch.tensor([[1, grid, grid]] * len(resized_images), dtype=torch.long)

        if videos:
            if codec_config is None:
                raise ValueError("Video samples require codec configuration.")
            video_outputs = [
                process_video_with_codec(video, processor=processor, config=codec_config) for video in videos
            ]
            sample["pixel_values_videos"] = torch.cat([item["pixel_values_videos"] for item in video_outputs], dim=0)
            sample["video_grid_thw"] = torch.cat([item["video_grid_thw"] for item in video_outputs], dim=0)
            sample["patch_positions"] = torch.cat([item["patch_positions"] for item in video_outputs], dim=0)
        ...
        if videos and codec_config is not None and codec_config.cv_reader_required:
            raise
...
def _resolve_image_processor_config(processor: Any) -> tuple[Any, int, int, int, int]:
    ...
    if hasattr(processor, "onevision_image_processor"):
        image_processor = processor.onevision_image_processor
        patch_size = int(getattr(processor, "onevision_patch_size", 14))
        merge_size = 1
        image_size = int(getattr(processor, "onevision_image_size", 448))
        return image_processor, patch_size, merge_size, image_size * image_size, image_size * image_size
...
def _normalize_message(message: dict[str, Any]) -> dict[str, str]:
    """Normalize a raw conversation message to the Video VLM chat schema.
...
def _process_message(
    ...
    *,
    ...
    video_iter = iter(()) if videos is None else videos
    cursor = 0
    while cursor < len(content):
        image_pos = content.find(image_placeholder, cursor)
        video_pos = content.find(video_placeholder, cursor)
        candidates = [(pos, "image") for pos in [image_pos] if pos >= 0]
        candidates.extend((pos, "video") for pos in [video_pos] if pos >= 0)
        if not candidates:
            tail = content[cursor:]
            if tail:
        ...
        if pos > cursor:
            blocks.append({"type": "text", "text": content[cursor:pos]})
        if kind == "image":
            try:
            ...
            try:
                video_value = next(video_iter)
                blocks.append({"type": "video", "video": video_value})
            except StopIteration as exc:
                raise ValueError("has more video placeholders than videos.") from exc
            cursor = pos + len(video_placeholder)
...
def _normalize_image_size(size_entry: Any) -> list[int]:
    """Parse Video VLM image metadata into ``[height, width]``.
...
def process_sample(
    ...
    *,
    ...
    try:
        ...
        if list(videos_iter):
            raise ValueError("has more videos than video placeholders.")
        ...
        use_onevision_images = hasattr(processor, "onevision_image_processor")
        for size in image_sizes:
            height, width = int(size[0]), int(size[1])
            if use_onevision_images:
                resized_height = resized_width = int(getattr(processor, "onevision_image_size", 448))
                adjusted_image_sizes.append([resized_height, resized_width])
                token_count = (resized_height // patch_size) * (resized_width // patch_size)
            else:
                resized_size = _smart_resize_image_size(
                    height,
                    width,
        ...
        video_token_count = int(codec_config.k_keep if codec_config is not None else 0)

        image_cursor = 0
        video_cursor = 0

        def _expand_image_placeholders(text: str) -> str:
            """Expand image/video placeholders to their visual token counts."""
            nonlocal image_cursor, video_cursor

            out: list[str] = []
            cursor = 0
            while cursor < len(text):
                image_pos = text.find(image_token, cursor)
                if IMAGE_TOKEN_PLACEHOLDER in text:
                    image_pos = text.find(IMAGE_TOKEN_PLACEHOLDER, cursor)
                video_pos = text.find(video_token, cursor)
                candidates = [(pos, "image") for pos in [image_pos] if pos >= 0]
                candidates.extend((pos, "video") for pos in [video_pos] if pos >= 0)
                if not candidates:
                ...
                if kind == "image":
                    ...
                    placeholder = (
                        ...
                        if IMAGE_TOKEN_PLACEHOLDER in text[pos:] and text.startswith(IMAGE_TOKEN_PLACEHOLDER, pos)
                        else image_token
                    )
                    out.append(image_token * token_count)
                    cursor = pos + len(placeholder)
                else:
                    if video_token_count <= 0:
                        raise ValueError("video placeholder requires codec_config.")
                    video_cursor += 1
                    out.append(video_token * video_token_count)
                    cursor = pos + len(video_token)
            return "".join(out)
        ...
        used_video_count = sum(
            1
            for message in rendered_messages
            for block in message.get("content", [])
            if isinstance(block, dict) and block.get("type") == "video"
        )
        if video_cursor != used_video_count:
            raise ValueError("video placeholders do not match rendered video blocks.")
        sample["images"] = images[:used_image_count]
        sample["adjusted_image_size"] = adjusted_image_sizes[:used_image_count]
        sample["videos"] = videos[:used_video_count]
        ...
        if not _VISION_TOKEN_IDS:
            for token in (VISION_START_TOKEN, VISION_END_TOKEN, image_token, video_token):
