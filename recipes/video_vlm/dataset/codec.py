"""On-the-fly codec patchification helpers for the Video VLM recipe."""
...
def _load_video_frames(video_path: str | os.PathLike[str], config: CodecPatchConfig) -> torch.Tensor:
    """Load uniformly sampled RGB frames with decord."""
    try:
        import decord
    except Exception as exc:
        raise RuntimeError("Video VLM on-the-fly codec preprocessing requires `decord`.") from exc

    reader = decord.VideoReader(str(video_path), num_threads=2, ctx=decord.cpu(0))
    if len(reader) <= 0:
        raise ValueError(f"video has no frames: {video_path}")
    frame_indices = _sample_frame_indices(len(reader), config.num_frames)

    frames = reader.get_batch(frame_indices).asnumpy()
    tensors = []
    for frame in frames:
        pil = Image.fromarray(frame).convert("RGB")
        tensor = tvF.pil_to_tensor(pil)
...
def _probe_video_frame_count(video_path: str | os.PathLike[str]) -> int:
    """Return the first video stream frame count, falling back to decord when needed."""
    command = [
        "ffprobe",
        "-v",
        "error",
    ...
    try:
        ...
        return int(len(decord.VideoReader(str(video_path), num_threads=1, ctx=decord.cpu(0))))
    except Exception as exc:
...
def _resolve_cv_reader_api():
    ...
    try:
        ...
        raise RuntimeError(
            "Video VLM codec residuals require `cv_reader` for H.264/H.265 support. "
...
def process_video_with_codec(
    ...
    *,
    ...
    if image_processor is None:
        raise ValueError("processor must expose `onevision_image_processor` for Video VLM codec preprocessing.")
