"""Training engine for the Video VLM recipe."""
...
class VideoVLMEngine(Engine):
    """Recipe-local engine for the Video VLM."""

    ConfigClass = VideoVLMConfig
    ...
    def __init__(self, config):
        """Initialize Video VLM-local distributed state and metric reducers."""
    ...
    def prepare_dataloader(self, workflow: str = "train"):
        ...
        """
        if workflow != "train":
            logger.warning(f"Video VLM engine does not support workflow '{workflow}'.")
            return
    ...
    def prepare_optimizer(self) -> torch.optim.Optimizer:
        ...
        if not trainable_parameters:
            raise ValueError("No trainable parameters found for the Video VLM recipe.")
