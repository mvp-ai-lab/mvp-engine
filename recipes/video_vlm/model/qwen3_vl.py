"""Qwen3-VL model helpers for the Video VLM recipe."""
...
class OneVisionVisualTower(nn.Module):
    """Qwen3-VL visual-tower adapter backed by OneVision-Encoder."""
    ...
    def dtype(self) -> torch.dtype:
        """Return the dtype used by the OneVision encoder."""
    ...
    def forward(
        ...
        **_: Any,
        ...
        """Encode image or codec-packed video tensors for Qwen placeholder insertion."""
...
def replace_visual_tower_with_onevision(model, model_config: Any):
    """Replace Qwen3-VL's visual tower with a OneVision-compatible adapter."""
    ...
    model.model.visual = OneVisionVisualTower(
        vision_encoder_name_or_path=model_config.vision_encoder_name_or_path,
        output_hidden_size=output_hidden_size,
        attn_implementation=model_config.attn_implementation,
        freeze_encoder=bool(model_config.freeze_vision_encoder),
    )
    return model


def patch_visual_feature_routing(model):
    """Route image/video tensors through OneVision and pass codec patch positions."""
...
def inject_model_flops_calculation(model):
    ...
    def calculate_model_flops(
        ...
        *,
        ...
        vit_flops = 0.0
        merger_flops = 0.0
        visual_grids = [grid for grid in (image_grid_thw, video_grid_thw) if grid is not None and grid.numel() > 0]
        if visual_grids:
            grid = torch.cat(
                [item.detach().to(device="cpu", dtype=torch.long).reshape(-1, 3) for item in visual_grids], dim=0
            )
            if torch.any(grid <= 0):
...
def inject_sum_loss_forward(model, *, chunk_size: int = 4096):
    ...
    def forward(
        ...
        **kwargs,
        ...
        self.model._video_vlm_patch_positions = patch_positions
        try:
            outputs = self.model(
                input_ids=input_ids,
            ...
            self.model._video_vlm_patch_positions = None
...
def build_qwen3_vl_model(model_config: Any):
    ...
    Args:
        model_config: Recipe model config (``VideoVLMModelConfig``) with load
            and freeze settings.
    ...
    model = replace_visual_tower_with_onevision(model, model_config)
