"""Structure tests for the Video VLM recipe.
...
def test_file_structure():
    """Validate the baseline Video VLM file layout.
    ...
    EXPECTED_FILES = [
        ...
        "tools/convert_llava_onevision_viewer.py",
        "utils/__init__.py",
...
def test_config_structure():
    """Validate all Video VLM YAML configs against the recipe schema.

    Each config is parsed through ``VideoVLMConfig`` and its engine name is
    ...
    from recipes.video_vlm.configs.schema import VideoVLMConfig
    ...
    try:
        for config_file in configs:
            if config_file.endswith(".yaml"):
                ...
                config = VideoVLMConfig.model_validate(
