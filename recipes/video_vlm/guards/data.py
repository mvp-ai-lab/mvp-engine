...
def build_empty_sample():
    ...
    The Video VLM data pipeline represents dropped examples as zero-length
...
class DataGuard(Assembler[Any, Any]):
    """Filter malformed Video VLM samples before expensive downstream processing.
    ...
    def __init__(
        ...
        """Configure which Video VLM sample checks are active."""
        super().__init__()
    ...
    def push(self, sample: Any) -> Iterable[Any]:
        ...
        Args:
            sample: A decoded Video VLM sample. Expected fields depend on which
                checks are enabled.
