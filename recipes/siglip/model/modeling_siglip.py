...
# use pip install transformers==4.55
import math
import warnings
...
class SiglipVisionModel(Qwen2_5_VisionTransformerPretrainedModel):
    config: Qwen2_5_VLVisionConfig
    _no_split_modules = ["Qwen2_5_VLVisionBlock"]

    def __init__(self, config: Qwen2_5_VLVisionConfig, *inputs, **kwargs):
...
#         # Store the activation for the layer
#         if isinstance(output, tuple):
#             activations[module] = output[0].detach()
#         elif isinstance(output, BaseModelOutput):
#             activations[module] = output.last_hidden_state.detach()
#         elif isinstance(output, BaseModelOutputWithPooling):
#             activations[module] = output.pooler_output.detach()
#         elif isinstance(output, SiglipOutput):
#             activations[module] = output.loss.detach()
#         else:
#             activations[module] = output.detach()

#     hooks = []

#     # Iterate over all layers and register hooks
...
# Function to recursively register hooks on bottom-level modules only
def register_hooks(model):
    ...
    # Iterate over all layers and submodules recursively
    def register_leaf_hooks(module):
        # If the module has no children (i.e., it's a leaf), register the hook
        if not list(module.children()):
            hook = module.register_forward_hook(hook_fn)
            hooks.append(hook)

        # If it has children, recursively call on each child
        for child in module.children():
            register_leaf_hooks(child)

    # Start with the root model and recursively register hooks on leaf modules
    register_leaf_hooks(model)
...
def main():
    ...
    config = SiglipConfig.from_text_vision_configs(text_config, vision_config)

    model = SiglipModel(config)
    model = model.cuda().to(torch.half)

    # tokenizer = AutoTokenizer.from_pretrained("google/siglip-so400m-patch14-384")
    tokenizer = AutoTokenizer.from_pretrained("google/siglip-base-patch16-224")

    # important: make sure to set padding="max_length" as that's how the model was trained
    inputs = tokenizer(["a photo of a cat", "a photo of a fox"], padding="max_length", return_tensors="pt")
    for k, v in inputs.items():
        inputs[k] = v.cuda()

    # pixel_values = torch.randn(2, 3, 224, 224).to(torch.bfloat16).npu()
    # torch.save(pixel_values.cpu(), "pixel_values.pth")
    fox_image = Image.open("./test_images/fox.jpg")
    fox_image = fox_image.resize(size=(224, 224))
    fox_image = T.PILToTensor()(fox_image)

    cat_image = Image.open("./test_images/cat.1.jpg")
    cat_image = cat_image.resize(size=(224, 224))
    cat_image = T.PILToTensor()(cat_image)

    pixel_values = torch.stack([fox_image, cat_image], dim=0).to(torch.half).cuda()

    activations, hooks = register_hooks(model)

    model(inputs["input_ids"], pixel_values, return_loss=True)

    # print('loss: ', loss)

    # After the forward pass, print activations of each layer
    for layer, activation in activations.items():
        print(f"Activation of layer {layer}: {activation.shape}")

    activations_numpy = {}
    counter = 0
    for layer, activation in activations.items():
        activations_numpy[str(layer) + "_" + str(counter)] = activation.cpu().float().numpy()
        counter = counter + 1

    # Save the activations dictionary as a NumPy object
    np.save("activations_gpu.npy", activations_numpy)
