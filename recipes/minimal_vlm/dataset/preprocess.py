...
_VISION_TOKEN_IDS: torch.Tensor | None = None
...
def process_sample(
    ...
    *,
    ...
    global _VISION_TOKEN_IDS
    if _VISION_TOKEN_IDS is None:
        vision_token_ids: set[int] = set()
        for token in (VISION_START_TOKEN, VISION_END_TOKEN, image_token):
            vision_token_ids.update(tokenizer(token, add_special_tokens=False)["input_ids"])
        _VISION_TOKEN_IDS = torch.tensor(sorted(vision_token_ids), dtype=torch.long)
    ...
    labels[torch.isin(input_ids, _VISION_TOKEN_IDS)] = ignore_index
