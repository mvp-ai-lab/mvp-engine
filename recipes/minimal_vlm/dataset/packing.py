"""Packing utilities for the minimal VLM recipe."""
...
class PackedSampleAssembler(Assembler[dict[str, Any], dict[str, Any]]):
    ...
    ``pack_buffer_size=-1`` enables streaming mode and drains on every push.
    A non-negative ``pack_buffer_size`` buffers samples first, packing from
    the pool with a global view once the buffer exceeds the limit.
    """
    ...
    def push(self, sample: dict[str, Any]) -> Iterable[dict[str, Any]]:
        """Consume one processed sample and yield any completed packed outputs."""
    ...
    def finish(self, *, drop_last: bool = False) -> Iterable[dict[str, Any]]:
        """Flush any buffered packing state at end of stream."""
    ...
    def _assign_pending_to_packs(self) -> list[dict[str, Any]]:
        """Try to assign all pending samples into open packs. Return any completed packs."""
        ...
        while self.pending_samples and made_progress:
            ...
            for idx in indices:
                ...
                # Inline best-fit / random selection over open packs.
            ...
            # Remove assigned indices in reverse to preserve ordering.
            for idx in sorted(assigned_indices, reverse=True):
                del pending[idx]

            # Flush packs that are exactly full.
            emitted.extend(self._flush_ready_packs())

        return emitted

    def _flush_ready_packs(self) -> list[dict[str, Any]]:
        """Close and emit any packs that are exactly full."""
        emitted: list[dict[str, Any]] = []
        if self.selection_strategy == "best_fit":
    ...
    def _close_most_filled_pack(self) -> dict[str, Any]:
        """Remove and emit the pack with the least remaining capacity."""
        if not self.open_packs:
    ...
    def _open_pack_from_pending(self) -> None:
        """Pop one pending sample and start a new open pack from it."""
        if not self.pending_samples:
            raise RuntimeError("Cannot open a pack from an empty sample pool.")

        if self.selection_strategy == "random":
            sample_index = self.rng.randrange(len(self.pending_samples))
        else:
            sample_index = 0
