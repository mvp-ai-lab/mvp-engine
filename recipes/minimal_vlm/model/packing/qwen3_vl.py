...
def _build_qwen3_vl_segment_position_ids(
    *,
    ...
    if bool((input_ids == video_token_id).any().item()):
        raise NotImplementedError("Packed minimal_vlm only supports text+image inputs, not video inputs.")
    ...
    for _ in range(image_count):
        image_start = input_tokens.index(image_token_id, segment_start)
        ...
        llm_position_chunks.append(torch.stack([t_index, h_index, w_index]) + text_length + next_position)

        segment_start = image_start + (llm_grid_t * llm_grid_h * llm_grid_w)

    if segment_start < len(input_tokens):
        next_position = int(llm_position_chunks[-1].max().item()) + 1 if llm_position_chunks else 0
        tail_length = len(input_tokens) - segment_start
        llm_position_chunks.append(
