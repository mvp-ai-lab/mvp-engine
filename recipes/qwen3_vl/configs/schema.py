"""Pydantic schema for the Qwen3-VL recipe."""
...
class Qwen3VLDataConfig(BaseModel):
    """Dataset and batching options for the Qwen3-VL recipe."""
...
class Qwen3VLGradientCheckpointingConfig(BaseModel):
...
class Qwen3VLCompileConfig(BaseModel):
    """Model compile options for the Qwen3-VL recipe."""
...
class Qwen3VLModelConfig(BaseModel):
    ...
    pretrained_model_name_or_path: str = "Qwen/Qwen3-VL-8B-Instruct"
    attn_implementation: Literal["eager", "sdpa", "flash_attention_2"] = "flash_attention_2"
    image_max_pixels: int | None = Field(None, ge=1)
    gradient_checkpointing: Qwen3VLGradientCheckpointingConfig = Field(
        default_factory=Qwen3VLGradientCheckpointingConfig
    )

    # Full-SFT defaults: train vision, projector, and language-model parameters.
...
class Qwen3VLOptimConfig(BaseOptimConfig):
    """Optimizer options extended with Qwen3-VL batch and loss-guard settings."""
...
class Qwen3VLLoopConfig(BaseLoopConfig):
    """Iteration-loop options for Qwen3-VL training."""
...
class Qwen3VLConfig(BaseEngineConfig):
    """Top-level Qwen3-VL recipe config."""

    model_config = ConfigDict(frozen=False, extra="allow")

    data: Qwen3VLDataConfig = Field(default_factory=Qwen3VLDataConfig)
    model: Qwen3VLModelConfig = Field(default_factory=Qwen3VLModelConfig)
    optim: Qwen3VLOptimConfig = Field(default_factory=Qwen3VLOptimConfig)
    loop: Qwen3VLLoopConfig = Field(default_factory=Qwen3VLLoopConfig)
