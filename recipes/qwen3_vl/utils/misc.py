"""Miscellaneous helpers for the Qwen3-VL recipe."""
...
def infer_total_steps(
    ...
    *,
    ...
    """Infer total optimization steps from one finite Qwen3-VL data pass."""
    ...
    if global_packed_samples <= 0:
        raise RuntimeError("Qwen3-VL step inference found no packed training samples.")
    ...
    if samples_per_optimization_step <= 0:
        raise RuntimeError(
            "Qwen3-VL step inference cannot infer steps with non-positive samples per optimization step."
        )
