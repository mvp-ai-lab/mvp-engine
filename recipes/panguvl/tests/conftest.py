"""Pytest configuration for PanguVL recipe-local tests."""
...
def pytest_addoption(parser: pytest.Parser) -> None:
    """Register command-line options consumed by PanguVL smoke tests."""
    parser.addoption(
        "--run-smoke",
        action="store_true",
        default=False,
        help="Run accelerator-backed smoke tests.",
    )
    parser.addoption(
        "--world-size",
        type=int,
...
def run_smoke(request: pytest.FixtureRequest) -> bool:
    """Return whether accelerator-backed smoke tests should run."""
    return request.config.getoption("--run-smoke")
