...
from mvp_dataset.core.types import StatefulAssembler

from recipes.panguvl.dataset import dataset as dataset_module
from recipes.panguvl.dataset.gate import (
...
def _packed_sample(length: int, *, offset: int = 0) -> dict[str, torch.Tensor]:
    values = torch.arange(offset, offset + length, dtype=torch.long)
    return {
        "input_ids": values,
        "attention_mask": torch.ones(length, dtype=torch.long),
        "labels": values.clone(),
...
def _normalize_output(value):
    ...
    if isinstance(value, dict):
        return {key: _normalize_output(item) for key, item in sorted(value.items())}
    if isinstance(value, list):
        return [_normalize_output(item) for item in value]
    return value


def test_invalid_sample_gate_assembler_is_stateful():
    """The invalid-sample gate must satisfy mvp_dataset's resumable assemble protocol."""
    assembler = InvalidSampleGateAssembler()
    valid_sample = _packed_sample(3)
    skipped_sample = {
    ...
    assert isinstance(assembler, StatefulAssembler)
    assert assembler.push(skipped_sample) == []
    assert assembler.push(valid_sample) == [valid_sample]
    assert assembler.state_dict() == {}

    restored = InvalidSampleGateAssembler()
    restored.load_state_dict(assembler.state_dict())
    assert restored.fingerprint() == assembler.fingerprint()


@pytest.mark.parametrize("selection_strategy", ["best_fit", "random"])
def test_packed_sample_assembler_resume_matches_continued_stream(selection_strategy):
    """Checkpointing a sample packer mid-stream should preserve future outputs."""
    lengths = [3, 9, 4, 7, 12, 1, 5, 6, 2, 8, 10, 2]
    kwargs = {
        "max_length": 12,
        "selection_strategy": selection_strategy,
        "open_pack_limit": 3,
    ...
    samples = [_packed_sample(length, offset=index * 100) for index, length in enumerate(lengths)]
    checkpoint_after = 6

    continued = PackedSampleAssembler(**kwargs)
    for sample in samples[:checkpoint_after]:
        list(continued.push(sample))
    state = continued.state_dict()

    continued_outputs = []
    for sample in samples[checkpoint_after:]:
        continued_outputs.extend(continued.push(sample))
    continued_outputs.extend(continued.finish())

    resumed = PackedSampleAssembler(**kwargs)
    resumed.load_state_dict(state)
    resumed_outputs = []
    for sample in samples[checkpoint_after:]:
        resumed_outputs.extend(resumed.push(sample))
    resumed_outputs.extend(resumed.finish())

    assert _normalize_output(resumed_outputs) == _normalize_output(continued_outputs)


@pytest.mark.parametrize("selection_strategy", ["best_fit", "random"])
def test_packed_length_assembler_resume_matches_continued_stream(selection_strategy):
    """The lightweight length packer should resume with the same decisions as the tensor packer."""
    lengths = [3, 9, 4, 7, 12, 1, 5, 6, 2, 8, 10, 2]
    kwargs = {
        "max_length": 12,
        "selection_strategy": selection_strategy,
    ...
    samples = [{"length": length} for length in lengths]
    checkpoint_after = 6

    continued = PackedLengthAssembler(**kwargs)
    for sample in samples[:checkpoint_after]:
        list(continued.push(sample))
    state = continued.state_dict()
    ...
    resumed = PackedLengthAssembler(**kwargs)
    resumed.load_state_dict(state)
    resumed_outputs = []
    for sample in samples[checkpoint_after:]:
        resumed_outputs.extend(resumed.push(sample))
    ...
    assert resumed_outputs == continued_outputs
    assert resumed.fingerprint() != PackedSampleAssembler(**kwargs).fingerprint()


def test_panguvl_assemble_stage_builds_iterator_without_unsupported_resume(tmp_path):
    """A PanguVL-style assemble stage should satisfy mvp_dataset's iterator contract."""
    parquet_path = tmp_path / "samples.parquet"
    pq.write_table(pa.Table.from_pylist([{"length": 3}, {"length": 0}, {"length": 2}]), parquet_path)

    def to_model_input(sample):
        length = int(sample["length"])
        return _packed_sample(length)

    dataset = (
        Dataset.from_source(
            "parquet",
            str(parquet_path),
            context=RuntimeContext(seed=5),
            resample=False,
        )
        .map(to_model_input)
        .assemble(build_invalid_sample_gate_assembler)
    )

    outputs = list(dataset)

    assert [int(output["input_ids"].numel()) for output in outputs] == [3, 2]
