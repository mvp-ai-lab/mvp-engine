    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            ...
            cu_seqlens_list = cu_seqlens.tolist() if isinstance(cu_seqlens, torch.Tensor) else cu_seqlens
            ...
            attn_output = torch_npu.npu_fusion_attention(
                ...
                inner_precise=0,
                sparse_mode=0,
                actual_seq_qlen=cu_seqlens_list,
                actual_seq_kvlen=cu_seqlens_list,
            )[0]
        else:
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if (
            ...
            npu_attn_mask = None
            if attention_mask is not None:
                npu_attn_mask = ~attention_mask.bool()
            elif bsz == 1 or q_len > 1:
                npu_attn_mask = (
                    torch.triu(torch.ones(q_len, q_len, dtype=torch.bool, device=query_states.device), diagonal=1)
                    .unsqueeze(0)
                    .unsqueeze(0)
                )
            attn_output = torch_npu.npu_fusion_attention(
                ...
                keep_prob=1.0 - (self.attention_dropout if self.training else 0.0),
                inner_precise=0,
                sparse_mode=0 if self.sliding_window is None else 2,
            )[0]
...
class OpenPanguVLModel(OpenPanguPreTrainedModel):
    ...
    def get_image_features(
        ...
        pixel_values = rescale_and_normalize(
            pixel_values, self.do_rescale, self.rescale_factor, self.do_normalize, self.image_mean, self.image_std
        )
        pixel_values = pixel_values.reshape(-1, self.channel * self.patch_size * self.patch_size)
