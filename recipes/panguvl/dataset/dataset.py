...
from .packing import PackedLengthAssembler, build_packed_sample_assembler
...
_LIGHTWEIGHT_SIZE_SOURCE_LOGGED: set[str] = set()
...
def align_messages_for_thinking(
    ...
    *,
    ...
    """Apply LF-private thinking-mode rewriting to assistant messages.
...
def process_sample(
    ...
    *,
    ...
    Args:
        ...
        image_placeholder: Placeholder token that marks image positions in text.
        ignore_index: Label value used to mask tokens out of the loss.
        thinking_mode: Thinking-tag mode aligned to LF-private.
...
def lightweight_process_sample(
    ...
    *,
    ...
    def normalize_image_sizes(value: Any, *, expected_count: int) -> list[tuple[int, int]] | None:
        """Return stored ``(height, width)`` pairs when the row provides valid metadata."""
        if value is None:
            return None
        if (
            expected_count == 1
            ...
            raise ValueError("has invalid `img_size` metadata.")

        if len(values) != expected_count:
            raise ValueError("has `img_size` metadata that does not match the image count.")
    ...
    try:
        ...
        image_sizes = normalize_image_sizes(sample.get("img_size"), expected_count=len(images))
        if image_sizes is not None:
            if "img_size" not in _LIGHTWEIGHT_SIZE_SOURCE_LOGGED:
                logger.info("PanguVL step inference: using stored img_size metadata for image token lengths.")
                _LIGHTWEIGHT_SIZE_SOURCE_LOGGED.add("img_size")
        else:
...
def build_dataset(
    ...
    *,
    ...
    if process_fn in {process_sample, lightweight_process_sample}:
        process_kwargs["thinking_mode"] = config.data.thinking_mode
...
def build_step_count_dataset(config: Any, *, processor: Any) -> Dataset:
    """Build a lightweight dataset for total-step inference without loading image bytes."""
    set_logger(logger)
    dataset_path_value = config.data.train_path
    if dataset_path_value is None:
        raise ValueError("Missing `data.train_path` for the PanguVL recipe.")
    ...
    dataset = Dataset.from_source(
        ...
        context=context,
        resample=False,
        columns=["conversations", "img_size", "images"],
    ).map(
        partial(
    ...
    if config.data.packing:
        dataset = dataset.map(lambda sample: {"length": int(sample["input_ids"].size(0))})
        dataset = dataset.assemble(
            lambda assemble_context: PackedLengthAssembler(
                max_length=int(config.data.max_seq_len),
                selection_strategy=config.data.packing_selection_strategy,
                open_pack_limit=int(config.data.packing_open_pack_limit),
                pack_buffer_size=int(config.data.packing_buffer_size),
