...
class PanguvlDataConfig(BaseModel):
    """Dataset and batching options for the PanguVL recipe."""

    model_config = ConfigDict(frozen=False, extra="forbid")

    train_path: str = "./data/panguvl/alignment_demo.jsonl"
    ...
    def validate_cache_dir(cls, value: str | None) -> str | None:
        """Normalize the optional cache directory path."""
        if value is None:
            return None
        if not isinstance(value, str):
...
class PanguvlGradientCheckpointingConfig(BaseModel):
    """Gradient checkpointing options passed to the PanguVL model."""
...
class PanguvlCompileConfig(BaseModel):
    """Model compile options for the PanguVL recipe."""
...
class PanguvlModelConfig(BaseModel):
    ...
    compile: PanguvlCompileConfig = Field(default_factory=PanguvlCompileConfig)
...
class PanguvlOptimConfig(BaseOptimConfig):
    """Optimizer options extended with PanguVL batch and loss-guard settings."""
...
class PanguvlLoopConfig(BaseLoopConfig):
    """Iteration-loop options for PanguVL training."""
...
class PanguvlConfig(BaseEngineConfig):
    """Top-level PanguVL recipe config."""
