"""Pytest configuration for qwen2_5_vl recipe-local tests."""
...
sys.path.insert(0, str(project_root / "recipes"))


def pytest_addoption(parser: pytest.Parser) -> None:
    """Register command-line options consumed by smoke tests."""
    parser.addoption("--run-smoke", action="store_true", default=False, help="Run the accelerator smoke test.")
    parser.addoption("--world-size", type=int, default=1, help="Distributed world size for smoke tests.")
    parser.addoption(
        "--config-override",
        action="append",
        default=[],
