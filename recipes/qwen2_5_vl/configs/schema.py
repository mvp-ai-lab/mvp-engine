"""Pydantic schema for the qwen2_5_vl recipe."""
...
class Qwen2_5VLDataConfig(BaseModel):
    """Dataset and batching options for the qwen2_5_vl recipe."""

    model_config = ConfigDict(frozen=False, extra="forbid")

    train_path: str = "./data/mvp-engine-vlm-dev-data/meta.json"
...
class Qwen2_5VLGradientCheckpointingConfig(BaseModel):
    """Gradient checkpointing options passed to the model."""
...
class Qwen2_5VLCompileConfig(BaseModel):
    """Model compile options for the recipe."""
...
class Qwen2_5VLModelConfig(BaseModel):
    """Model loading, precision compatibility, and freeze-policy options."""

    model_config = ConfigDict(frozen=False, extra="forbid")

    pretrained_model_name_or_path: str = "Qwen/Qwen2.5-VL-7B-Instruct"
    attn_implementation: Literal["eager", "sdpa", "flash_attention_2"] = "flash_attention_2"
    image_max_pixels: int | None = Field(None, ge=1)
    gradient_checkpointing: Qwen2_5VLGradientCheckpointingConfig = Field(
        default_factory=Qwen2_5VLGradientCheckpointingConfig
    )
    freeze_vit: bool = False
    freeze_projector: bool = False
    freeze_llm: bool = False
    compile: Qwen2_5VLCompileConfig = Field(default_factory=Qwen2_5VLCompileConfig)
...
class Qwen2_5VLOptimConfig(BaseOptimConfig):
...
class Qwen2_5VLLoopConfig(BaseLoopConfig):
    """Iteration-loop options for qwen2_5_vl training."""
...
class Qwen2_5VLConfig(BaseEngineConfig):
    """Top-level qwen2_5_vl recipe config."""

    model_config = ConfigDict(frozen=False, extra="allow")

    data: Qwen2_5VLDataConfig = Field(default_factory=Qwen2_5VLDataConfig)
    model: Qwen2_5VLModelConfig = Field(default_factory=Qwen2_5VLModelConfig)
    optim: Qwen2_5VLOptimConfig = Field(default_factory=Qwen2_5VLOptimConfig)
    loop: Qwen2_5VLLoopConfig = Field(default_factory=Qwen2_5VLLoopConfig)
