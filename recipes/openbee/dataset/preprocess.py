...
def convert_images_to_pixel_values(sample: dict[str, Any], *, processor: Any) -> dict[str, Any]:
    ...
    if not images:
        return sample
    if len(images) != len(adjusted_sizes):
        raise ValueError(
            "Image sample has mismatched images and adjusted_image_size counts: "
            f"{len(images)} images vs {len(adjusted_sizes)} sizes."
        )
    ...
    def _load_resized_image_tensor(image: Any, target_size: list[int]) -> torch.Tensor:
        if (
            ...
            or any(int(dimension) <= 0 for dimension in target_size)
        ):
            raise ValueError(f"Image sample has invalid adjusted_image_size: {target_size!r}.")

        height, width = int(target_size[0]), int(target_size[1])
        resolved_image = process_image(image)
        if isinstance(resolved_image, Image.Image):
            pil_image = resolved_image.convert("RGB")
        elif isinstance(resolved_image, str):
            with Image.open(resolved_image) as opened:
                pil_image = opened.convert("RGB").copy()
        else:
            raise ValueError(f"contains an invalid resolved image value: {type(resolved_image).__name__}.")
...
def process_sample(
    ...
    *,
    ...
    try:
        ...
        simple_info(f"Skipping sample with error: {sample}/n{exc}", level="warning")
