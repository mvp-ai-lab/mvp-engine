...
- 当目标 recipe 自己带有较重的 engine / dataset / checkpoint 定制，并且明显落后于当前共享契约时，读取 `references/tomatovit-parallel-refactor-example.md`。
- 当你需要一个已经基本对齐当前共享层的控制组，来判断问题来自 shared stack 还是 recipe-local 扩展时，读取 `references/vit-classification-baseline-example.md`。
