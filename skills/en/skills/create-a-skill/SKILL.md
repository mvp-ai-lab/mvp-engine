---
name: create-a-skill
description: Create or update a repo skill using the canonical skill format. Use when the user asks to add a new skill, rewrite an existing skill, or normalize a skill directory and mirror pair.
---
...
- The target skill path, including language and category.
...
- Any existing mirror directory under `skills/en/` or `skills/zh-cn/`.
...
- For a new skill:
  - create the mirrored directories under `skills/en/` and `skills/zh-cn/`
  - scaffold `SKILL.md` from the base template
  - add `references/` or `scripts/` only if the workflow truly needs them
- For an existing skill:
  - read the current `SKILL.md`, mirror file, and any local `references/` or `scripts/`
  - identify structural gaps:
    - missing YAML front matter
    - mismatched `name`
    - wrong language under `en/` or `zh-cn/`
...
- Default to mirrored directories:
...
- Start with YAML front matter and keep exactly these fields:
  - `name`
  - `description`
- Keep `name` identical across language mirrors and aligned with the directory name.
...
### 7. Keep the mirror pair aligned

- English content belongs under `skills/en/`; Chinese content belongs under `skills/zh-cn/`.
- Keep the structure and meaning aligned across both mirrors.
- Translation does not need to be literal, but the workflow, routing rules, and validation bar must match.

## Validation

- Confirm the final `SKILL.md` has valid front matter with only `name` and `description`.
- Confirm a new skill has both mirrored directories and the required `SKILL.md` files.
...
- Read [references/base-template.md](/home/c84391361/projects/mvp-engine/skills/en/skills/create-a-skill/references/base-template.md) when you are creating a new skill from scratch or doing a clean rewrite of a weak existing one.
