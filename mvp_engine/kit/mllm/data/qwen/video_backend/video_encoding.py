...
# A frame sampler maps (probe metadata, requested frame count) -> selected indices.
# Defaults to uniform sampling (``sample_frame_indices``); a recipe can inject a
# different strategy without editing the kit.
FrameSampler = Callable[[VideoMeta, int], list[int]]
...
def load_dense_video_frames(
    video: str | Path, config: DenseVideoConfig, *, sampler: FrameSampler | None = None
) -> torch.Tensor:
    """Decode sampled frames into a fixed-size ``[T,C,H,W]`` tensor."""
    config.validate()
    sampler = sampler or sample_frame_indices
    video_path = str(Path(video).expanduser().resolve())
    meta = probe_video(video_path)
    indices = sampler(meta, config.num_frames)
...
def process_video_with_dense_frames(
    ...
    *,
    ...
    """Decode one sampled video into a dense visual-token sequence."""
    frames = load_dense_video_frames(video, config, sampler=sampler)
...
def process_video_with_keyframe_lowres(
    ...
    *,
    ...
    frames, keyframe_mask = load_keyframe_lowres_video_frames(video, config, sampler=sampler)
    image_processor = getattr(processor, "onevision_image_processor", getattr(processor, "image_processor", None))
    if image_processor is None:
        raise ValueError("processor must expose `onevision_image_processor` for video MLLM preprocessing.")
