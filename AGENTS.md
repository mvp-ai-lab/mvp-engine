...
- `mvp_engine/skills/`: agent skills — structured guides for tasks that have clear patterns but cannot be generalized into a single API (for example gradient checkpointing, FSDP wrap policies). Organized by language (`en/`, `cn/`) and category (`training/`, `parallel/`, `model/`, `data/`, `debug/`, `recipe/`). See `mvp_engine/skills/README.md` for overview and `mvp_engine/skills/en/README.md` or `mvp_engine/skills/cn/README.md` for design rationale.
